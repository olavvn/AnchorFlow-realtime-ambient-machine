...
# resolve experiment name / work directory (non-interactive, Colab-friendly)
exp_name = args.exp_name or time.strftime("ambient_%Y%m%d_%H%M%S")
checkpoint_folder = os.path.join(args.ckpt_dir, exp_name)
if os.path.exists(checkpoint_folder) and not args.overwrite:
    exp_name += time.strftime("_%H%M%S")
    checkpoint_folder = os.path.join(args.ckpt_dir, exp_name)

os.makedirs(checkpoint_folder, exist_ok=True)
os.makedirs(os.path.join(checkpoint_folder, "script"), exist_ok=True)
os.makedirs(os.path.join(checkpoint_folder, "script", "preprocess"), exist_ok=True)
os.makedirs(os.path.join(checkpoint_folder, "log"), exist_ok=True)
# copy scripts
...
log_link = "logs/log_{}.txt".format(exp_name)
try:
    if os.path.exists(log_link):
        os.remove(log_link)
    os.link(mylogger.filepath, log_link)
except OSError:
    pass  # hard links unsupported (e.g., Colab/Drive); skip the convenience link
...
# dataset
train_dataset = getMusicDataset(pkl_path=os.path.join(args.data_dir, "train_seg2_512.pkl"),
                                args=args,
                                vocab=myvocab,
                                augment=True)


val_dataset = getMusicDataset(pkl_path=os.path.join(args.data_dir, "val_seg2_512.pkl"),
                                args=args,
                                vocab=myvocab,
                                augment=False)
