...
class MusicDataset(Dataset):
    def __init__(self,data,max_seq_len,vocab = None,aug=None,
                 augment=True,pitch_aug=5,time_stretch=0.1):
        ...
        # data augmentation (train only): random transposition + time-stretch
        self.augment = augment           # set False for validation
        self.pitch_aug = pitch_aug       # +/- semitones
        self.time_stretch = time_stretch # +/- fraction (0.1 -> 0.9~1.1x)
    ...
    def data_pitch_augment(self,src_seq,tgt_seq):
        ...
        if len(all_pitches) == 0 :
            return deepcopy(src_seq), deepcopy(tgt_seq)

        # transpose by +/- self.pitch_aug semitones, kept within valid 1~127 range
        lo = max(1 - min(all_pitches), -self.pitch_aug)
        hi = min(127 - max(all_pitches),  self.pitch_aug)
        pitch_offset = np.random.randint(lo, hi + 1) if lo <= hi else 0
    ...
    def time_stretch_seq(self, seq, factor):
        """scale every Time-Shift and Note-Duration token by `factor` (length-preserving)."""
        ts_max = int(self.vocab._timeshift_bins[-1])
        dur_max = int(self.vocab._duration_bins[-1])
        out = []
        for tid in seq:
            tok = self.id2event[tid]
            if tok.startswith("Time-Shift"):
                k = max(1, min(int(round(int(tok.split("_")[1]) * factor)), ts_max))
                out.append(self.event2id["Time-Shift_{}".format(k)])
            elif tok.startswith("Note-Duration"):
                head, d = tok.rsplit("_", 1)
                d = max(1, min(int(round(int(d) * factor)), dur_max))
                out.append(self.event2id["{}_{}".format(head, d)])
            else:
                out.append(tid)
        return out
    ...
    def __getitem__(self, index):
        ...
        # data augmentation (train only): transposition + time-stretch, same factor for src/tgt
        if self.augment:
            src, tgt = self.data_pitch_augment(src_seq=self.data[index]["src"], tgt_seq=self.data[index]["tgt"])
            f = 1.0 + np.random.uniform(-self.time_stretch, self.time_stretch)
            src = self.time_stretch_seq(src, f)
            tgt = self.time_stretch_seq(tgt, f)
        else:
            src = deepcopy(self.data[index]["src"])
...
def getMusicDataset(pkl_path,args,vocab,augment=True):
    ...
    Args:
        ...
        augment (bool): apply train-time augmentation (set False for validation)
    ...
    dataset = MusicDataset(data=train_data,max_seq_len=args.max_len,vocab=vocab,augment=augment)
